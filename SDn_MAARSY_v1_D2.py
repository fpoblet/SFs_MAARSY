...
cbar = plt.colorbar(label='(m$^2$s$^{-2}$)')
cbar.set_label(label='(m$^2$s$^{-2}$)', rotation=270, labelpad=15)

# Set specific contour levels
contour_levels = [1, 10, 100, 500]
contours = plt.contour(X, Y, Z_sf, levels=contour_levels, colors='navy', linewidths=0.5)
plt.clabel(contours, fmt='%1.2f m$^2$s$^{-2}$', colors='navy', fontsize=12)
...
plt.xlabel(r'$\tau$ (s)')
plt.ylabel(r'Binned $\bar{V}_{m,n}(\tau) \times \tau$ (km)')
plt.title(r'$D_2(\tau)$ - H: %1.1f km' %(H[ihs[ihh]]))  # Adjust the value accordingly
...
for ih in range(len(ihs)):
    ...
    popt, pcov = curve_fit(second_order_fit, bin_edges[(~np.isnan(DQ_loc)) & (bin_edges <= 1000*1000)]/1000, np.abs(DQ_loc[(~np.isnan(DQ_loc)) & (bin_edges <= 1000*1000)]))
    ...
    filename = "C:/Poblet/IAP/work/N-order_sf/figuritas/v_2/v_2_15min_res_2019-2022/with_fits_filtered_by_counts_0.6/D2_s_MAARSY{:02d}.jpg".format(ih)
...
with imageio.get_writer('C:/Poblet/IAP/work/N-order_sf/figuritas/v_2/v_2_15min_res_2019-2022/with_fits_filtered_by_counts_0.6/figures.gif', mode='I', duration=0.5) as writer:
